...
public class GoofyExplosion extends Explosion {
    ...
    public GoofyExplosion(World world, @Nullable Entity entity, double x, double y, double z, float power, boolean createFire, Explosion.DestructionType destructionType, List<BlockPos> affectedBlocks, float multiplier) {
        this(world, entity, x, y, z, power, createFire, destructionType,multiplier);
        this.affectedBlocks.addAll(affectedBlocks);
    }

    public GoofyExplosion(World world, @Nullable Entity entity, double x, double y, double z, float power, boolean createFire, Explosion.DestructionType destructionType, float multiplier) {
        this(world, entity, (DamageSource)null, (ExplosionBehavior)null, x, y, z, power, createFire, destructionType,multiplier);
    }
        ...
        if (bl) {
            ...
            for (BlockPos blockPos : this.affectedBlocks) {
                ...
                if (block.shouldDropItemsOnExplosion(this) && (world = this.world) instanceof ServerWorld) {
                    ...
                    blockState.onStacksDropped(serverWorld, blockPos, ItemStack.EMPTY, bl2);
                    blockState.getDroppedStacks(builder).forEach(stack -> GoofyExplosion.tryMergeStack(objectArrayList, stack, blockPos2));
                }
                this.world.setBlockState(blockPos, Blocks.AIR.getDefaultState(), Block.NOTIFY_ALL);
                block.onDestroyedByExplosion(this.world, blockPos, this);
                this.world.getProfiler().pop();
